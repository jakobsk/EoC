...
plot_freq_reverse <- (
  ggplot(frequencies_top_n, aes(Diagnostic, freq, fill = factor(x = Country, levels = top_n_countries)))
  + geom_bar(position = "fill", stat = "identity")
  + scale_y_continuous(label = percent)
  + ggtitle(paste0("Distribution of infections of top ", botnets_n, " botnets in top ", countries_n , " countries (%)"), subtitle = "Countries ordered by total size (descending)")
  + labs(y="Infections (%)", fill = "Country")
  + theme_few()
  + scale_fill_manual(
    values=c(
      "#3366CC", "#DC3912", "#FF9900", "#109618", "#990099", "#0099C6", "#DD4477", "#66AA00", "#B82E2E", "#316395",
